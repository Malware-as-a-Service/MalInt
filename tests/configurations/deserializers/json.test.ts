...
import { repository, invalidRepository, invalidServerSideServer, serverSideServer, invalidServerSideMalware, serverSideMalware } from "../types";
...
test("Return Validation error on invalid repository object", () => {
    const result = new Json().deserializeRepository(JSON.stringify(invalidRepository));
    expect(result.isErr()).toBe(true);
...
test("Return Validation error on invalid server side server object", () => {
    const result = new Json().deserializeServerSideServer(JSON.stringify(invalidServerSideServer));
    expect(result.isErr()).toBe(true);
...
test("Deserialize server side server configuration", () => {
    const result = new Json().deserializeServerSideServer(JSON.stringify(serverSideServer));
...
test("Return Validation error on invalid server side malware object", () => {
    const result = new Json().deserializeServerSideMalware(JSON.stringify(invalidServerSideMalware));
    expect(result.isErr()).toBe(true);
...
test("Deserialize server side malware configuration", () => {
    const result = new Json().deserializeServerSideMalware(JSON.stringify(serverSideMalware));
