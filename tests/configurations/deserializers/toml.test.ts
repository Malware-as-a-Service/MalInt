...
test("Return Validation error on invalid server side server object", () => {
	const result = new Toml().deserializeServerSideServer(
		stringify(invalidServerSideServer),
	);
	expect(result.isErr()).toBe(true);
...
test("Return Validation error on invalid server side malware object", () => {
	const result = new Toml().deserializeServerSideMalware(
		stringify(invalidServerSideMalware),
	);
	expect(result.isErr()).toBe(true);
